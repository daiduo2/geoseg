#!/usr/bin/env python3
"""Brute-force residual-minimizing color correction within a local window.

For each label that exceeds the residual threshold, instead of directly using the
median color, we:
1. Take the median color of the original pixels as the search center.
2. Brute-force sample RGB colors in a window around that center.
3. Pick the color that minimizes the mean L2 RGB residual for that label.

This is more robust than a single median when the label contains mixed pixels
(e.g. PM text + geology): the median may land on an outlier, while a local
search can find a nearby color that gives lower overall residual.
"""
...
OUT_DIR = GLOBAL_DIR / "brute_force_correction"
...
# Search window around the median color.
SEARCH_RANGE = 30
SEARCH_STEP = 3
...
def brute_force_optimal_color(
    ...
    """Find the color that minimizes mean L2 residual for the label."""
    pixels = original[labels == lbl].astype(np.float32)
    median = np.median(pixels, axis=0).astype(np.uint8)

    # Coarse search around median.
    coarse_best, _ = _search_best_color(pixels, median, SEARCH_RANGE, SEARCH_STEP)
...
def assemble_grid(
    ...
    header_labels = ["Original", "Global mask", "Global abs RGB diff", "BF-corrected mask", "BF-corrected abs RGB diff"]
    n_rows = len(rows)
...
def main() -> None:
    ...
    grid = assemble_grid(
        rows,
        f"Brute-force residual correction (threshold={RESIDUAL_THRESHOLD}, range=±{SEARCH_RANGE})",
        profile_labels,
        row_metrics,
        row_corrections,
    )
    grid_path = OUT_DIR / "abs_rgb_diff_brute_force_v12.jpg"
