"""Pipeline: felzenszwalb initial segmentation + adaptive post-merge.

Post-merge strategy:
1. Compute mean color + boundary gradient for each adjacent label pair
2. Merge score = color_dist / (mean_grad + eps) — lower = more likely merge
3. Iteratively merge lowest-score pairs until target layer count reached

This removes gradient artifacts while preserving real boundaries.
"""
...
def adaptive_post_merge(label_img: np.ndarray, image: np.ndarray,
                        small_area_ratio: float = 0.02,
                        max_score: float = 1.0,
                        max_color_dist: float = 35.0) -> np.ndarray:
    """Two-phase post-merge: remove small fragments, then conservative merge.

    Phase 1: merge regions smaller than small_area_ratio to nearest neighbor.
    Phase 2: merge adjacent pairs with score < max_score AND color_dist < max_color_dist.
    """
    h, w = label_img.shape
    ...
    # Phase 1: merge small fragments
    while True:
        unique, counts = np.unique(result, return_counts=True)
        mean_colors = {lbl: image[result == lbl].mean(axis=0) for lbl in unique}
        small = unique[counts < total * small_area_ratio]
        if len(small) == 0:
            break
        for small_lbl in small:
            ...
            # Merge to nearest neighbor by color
            min_dist = float('inf')
            nearest = None
    ...
    # Phase 2: conservative merge of color-similar weak boundaries
    while True:
        ...
        for pair, grads in pair_data.items():
            ...
            score = color_dist / (mean_grad + 1e-3)
            scores.append((score, pair, color_dist, mean_grad))

        scores.sort(key=lambda x: x[0])
        best_score, best_pair, cd, mg = scores[0]

        if best_score >= max_score or cd >= max_color_dist:
            break

        print(f"  merging {best_pair}: score={best_score:.2f}, color_dist={cd:.1f}, grad={mg:.2f}")
        i, j = best_pair
        result[result == j] = i
        renumber()
...
def process_panel(image_path: Path) -> dict:
    ...
    # Initial felzenszwalb segmentation
    labels = felzenszwalb(cleaned, scale=500, sigma=1.0, min_size=100)
    n_init = len(np.unique(labels))
    print(f"  initial: {n_init} labels")

    # Adaptive post-merge
    labels = adaptive_post_merge(labels, cleaned)
...
def main():
    ...
    out = base / "result_post_merge.png"
