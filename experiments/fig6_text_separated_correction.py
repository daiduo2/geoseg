#!/usr/bin/env python3
"""Separate text/annotation noise from geological labels.

After splitting label 12 into PM/text (100) and geology (101), we still see
letter-shaped noise (e.g. 'M') embedded in geological layers. This script:
1. Builds a text mask from very dark / very bright original pixels.
2. Reassigns those pixels inside geological labels to a new text label (200).
3. Keeps label 100 (PM/text) unchanged.
4. Runs brute-force residual correction only on geological labels.

Goal: geological layers are pure colors with low residual; text/noise layers
are allowed to have high residual.
"""
...
OUT_DIR = GLOBAL_DIR / "text_separated_correction"
...
# Pixels darker/brighter than these are treated as text/annotation noise.
DARK_THRESHOLD = 55
...
def detect_text_mask(original: np.ndarray) -> np.ndarray:
    gray = original.mean(axis=2)
    return (gray < DARK_THRESHOLD) | (gray > BRIGHT_THRESHOLD)


def separate_text_from_geology(labels: np.ndarray, original: np.ndarray) -> tuple[np.ndarray, int]:
    text_mask = detect_text_mask(original)
    # Only reassign text pixels that are inside existing labels (not background).
    # Keep label 100 (PM/text) and label 0 (sediment, often bright red) as is.
    inside_labels = labels > 0
    text_pixels = text_mask & inside_labels
    new_labels = labels.copy()
    new_labels[text_pixels] = TEXT_LABEL
    return new_labels, int(text_pixels.sum())


def make_palette(base_palette: np.ndarray, max_label: int, original: np.ndarray, labels: np.ndarray) -> np.ndarray:
    size = max(len(base_palette), max_label + 1, PM_LABEL + 1, GEOLOGY_LABEL + 1, TEXT_LABEL + 1)
    ...
    # Color text/noise label with the median of detected text pixels so it is
    # visible but does not contaminate geological layers.
    text_mask = labels == TEXT_LABEL
    if text_mask.any():
        palette[TEXT_LABEL] = np.median(original[text_mask], axis=0).astype(np.uint8)
    return palette
...
def iterative_correct(
    ...
    palette = make_palette(base_palette, max_label, original, labels)
    history: list[dict[int, float]] = []
    corrected_labels: set[int] = set()

    text_labels = {PM_LABEL, TEXT_LABEL}
...
def assemble_grid(
    ...
    header_labels = ["Original", "Global mask", "Global abs RGB diff", "Text-separated mask", "Text-separated abs RGB diff"]
    n_rows = len(rows)
...
def main() -> None:
    ...
    for panel_id in PROFILES:
        ...
        global_mask = render_mask(labels, make_palette(base_palette, int(np.max(labels)), original, labels))
        ...
        separated_labels, text_count = separate_text_from_geology(split_labels, original)
        corrected_palette, history, corrected = iterative_correct(
            separated_labels, original, base_palette, RESIDUAL_THRESHOLD, MAX_ITERATIONS
        )
        corrected_mask = render_mask(separated_labels, corrected_palette)
        ...
        print(f"{panel_id}: text_pixels={text_count}, corrected {corrected}, L2 {global_l2:.1f} -> {corrected_l2:.1f}")

    grid = assemble_grid(
        rows,
        f"Text-separated correction (threshold={RESIDUAL_THRESHOLD})",
        profile_labels,
        row_metrics,
        row_corrections,
    )
    grid_path = OUT_DIR / "abs_rgb_diff_text_separated_v14.jpg"
