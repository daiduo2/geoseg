...
# 组装层
uv run python -m geoseg.controller_demo            # 单图端到端流水线
uv run python -m geoseg.batch_processor_demo        # 批量目录处理
uv run python -m geoseg.pipeline_interfaces_demo    # 接口契约 demo
...
- `src/geoseg/modules/segment_engines/router.py` 的 `select_engine` + `_RETRY_CHAIN`（硬编码路由）— agent 自主决策（`sandbox-segment` skill）
