...
def per_label_metrics(
    ...
    """Compute boundary_alignment and fragment stats per label region.

    Useful for regional audit: agent can see which specific layers have
    good boundary alignment vs which need retry.

    Returns:
        {label_id: {"boundary_alignment": float, "area_fraction": float,
                     "has_tiny_fragments": bool}}
    """
    from skimage.color import rgb2gray
    from skimage.filters import sobel
    ...
    targets = label_ids if label_ids is not None else sorted(set(labels.flatten()) - {0})
    ...
    for lbl in targets:
        ...
        # Boundary alignment for this label only
        lbl_boundary = seg_boundaries & mask
        aligned = (lbl_boundary & edge_mask).sum()
        total = lbl_boundary.sum()
        ba = float(aligned / total) if total > 0 else 0.0

        area_frac = float(mask.sum() / total_area)

        # Check tiny fragments within this label
        cc = label(mask, connectivity=2)
        regions = regionprops(cc)
        min_area = max(30, int(total_area * 0.003))
        has_tiny = any(r.area < min_area for r in regions)

        result[int(lbl)] = {
            "boundary_alignment": round(ba, 4),
            "area_fraction": round(area_frac, 4),
            "has_tiny_fragments": has_tiny,
        }
...
def compute_all(
    ...
    return {
        ...
        "per_label": per_label_metrics(labels, img_rgb),
    }
