...
class FigureStatus(str, Enum):
    ...
    SEGMENTED = "segmented"
    NEEDS_EDIT = "needs_edit"  # user flagged for napari editing
    AUDIT_FAILED = "audit_failed"  # failed visual-audit hard-reject gates
    REVIEWED = "reviewed"
    EXPORTED = "exported"
    SKIPPED = "skipped"
    ERROR = "error"
...
class NapariEditRecord(BaseModel):
    """Record of a napari editing session."""

    edited_labels_path: str
...
class SegmentationRecord(BaseModel):
    ...
    overlay_path: str
    labels_path: str
    edited_labels_path: str | None = None
    shapes_path: str | None = None
    napari_edited: bool = False
    attempts: list[SegmentationAttempt] = Field(default_factory=list)
    user_feedback: list[str] = Field(default_factory=list)
    napari_edit: NapariEditRecord | None = None
    audit: dict | None = None  # visual-audit hard-reject result
...
def get_summary(state: SessionState) -> dict[str, Any]:
    ...
    return {
        ...
        "needs_edit": by_status.get(FigureStatus.NEEDS_EDIT.value, 0),
        "audit_failed": by_status.get(FigureStatus.AUDIT_FAILED.value, 0),
...
def list_needs_napari_edit(state: SessionState) -> list[FigureEntry]:
    """Figures flagged for napari editing during batch review."""
    return [e for e in state.workset if e.status == FigureStatus.NEEDS_EDIT]


def list_audit_failed(state: SessionState) -> list[FigureEntry]:
    """Figures that failed visual-audit hard-reject gates."""
    return [e for e in state.workset if e.status == FigureStatus.AUDIT_FAILED]
