...
> 代码库地图见 `docs/CODEBASE.md`，skill 索引见 `.claude/skills/README.md`。

## Current Architecture

- 源码位于 `src/geoseg/`，测试位于 `tests/`，示例位于 `examples/`，实验位于 `experiments/`。
- Pipeline 由 Claude Code skill 驱动：agent 直接读取图像做语义/视觉判断，Python 只提供确定性工具函数。
...
- 所有语义判断和视觉审阅必须由 Claude Code agent 直接读图完成。
...
- 更新 `docs/CODEBASE.md` 或现有模块 `CLAUDE.md`，不要散落新的长期说明文件。
...
- 不新增长期 markdown，除非归入 `docs/CODEBASE.md`、`docs/DESIGN.md`、现有 skill 文档或模块 `CLAUDE.md`。
