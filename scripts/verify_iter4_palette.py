#!/usr/bin/env python3
"""Verify iteration 4 palette adjustments and revert entries that worsened color fidelity."""
...
def main():
    ...
    iter3_dir = ref_root / "workflow_color_correction" / "iter_3_verified"
    out_dir = ref_root / "workflow_color_correction" / "iter_4_verified"
    ...
    iter4_changes = state["adjustments"][3]["changes"]
    ...
    for profile in profiles:
        ...
        iter3_mask = load_image(iter3_dir / profile / "mask_corrected.jpg")
        iter3_mask = iter3_mask[:h, :w]

        current = np.array(state["palettes"][profile]).copy()
        original_lab = rgb2lab(original)

        for change in iter4_changes:
            if change["profile"] != profile:
                continue
            lab = int(change["label"])
            suggested = np.array(change["suggested_rgb"])
            before_rgb = np.array(sample_label_rgb(iter3_mask, labels, lab))
        ...
        iter3_l2[profile] = mean_rgb_l2(original, iter3_mask, valid)
    ...
    fig.suptitle(
        "Iteration 4 verification: iter 3 verified vs. iter 4 verified (worse entries reverted)",
        fontsize=12,
    )
    ...
    for r, profile in enumerate(profiles):
        ...
        iter3_mask = load_image(iter3_dir / profile / "mask_corrected.jpg")[:h, :w]
        current = np.array(final_palettes[profile])

        verified_mask = render_mask(labels, current)
        iter3_diff = render_diff(original, iter3_mask, text_mask)
        verified_diff = render_diff(original, verified_mask, text_mask)

        images = [original, iter3_mask, verified_mask, iter3_diff, verified_diff]
        titles = [
            f"{profile}\nOriginal",
            f"Iter 3 verified\nL2={iter3_l2[profile]:.1f}",
