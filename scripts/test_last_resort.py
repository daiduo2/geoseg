#!/usr/bin/env python3
"""Last resort: test pinntomo and tomography panels that scored high before."""
...
AUDIT_DIR = Path("runs/new_papers_vlm/quality_review_audit_lastresort")
...
def process(img_path: Path, desc: str, n_hint: int, engine: str | None = None, panel_id: int | None = None) -> dict | None:
    ...
    out_name = f"{desc.replace(' ', '_').replace('/', '_')}_n{n_hint}_{engine_str}_{n_found}l_lastresort.png"
...
def main() -> None:
    ...
    # Pinntomo page4 panel 0 - previously 0.80 manual_fix
    for n in [5, 6, 7]:
        for eng in ["kmeans_full", "edge_grow"]:
            r = process(Path("papers_new/to_process/pinntomo_2021/pinntomo_2021_page4_render.png"), "pinntomo p4 panel0", n, eng, 0)
            if r:
                all_results.append(r)
                print(f"pinntomo p4 p0 n={n} eng={eng}: score={r.get('score', 'ERR')} rec={r.get('recommendation', 'ERR')}")

    # Pinntomo page4 panel 2 - slightly larger (305x305)
    for n in [5, 6]:
        for eng in ["kmeans_full", "edge_grow"]:
            r = process(Path("papers_new/to_process/pinntomo_2021/pinntomo_2021_page4_render.png"), "pinntomo p4 panel2", n, eng, 2)
            if r:
                all_results.append(r)
                print(f"pinntomo p4 p2 n={n} eng={eng}: score={r.get('score', 'ERR')} rec={r.get('recommendation', 'ERR')}")

    # Tomography page9_img3 panel 1 - previously 0.65 manual_fix with panel 2
    for n in [4, 5]:
        for eng in ["edge_grow", "kmeans_full"]:
            r = process(Path("papers_new/to_process/tomography_review_2024/tomography_review_2024_page9_img3.png"), "tomography p9i3", n, eng, 1)
            if r:
                all_results.append(r)
                print(f"tomography p9i3 p1 n={n} eng={eng}: score={r.get('score', 'ERR')} rec={r.get('recommendation', 'ERR')}")

    report_path = AUDIT_DIR / "lastresort_report.json"
