#!/usr/bin/env python3
"""Verify iteration 16 palette adjustments and revert entries that worsened color fidelity."""
...
def main():
    ...
    before_dir = ref_root / "workflow_color_correction" / "iter_15_verified"
    after_dir = ref_root / "workflow_color_correction" / "iter_16"
    out_dir = ref_root / "workflow_color_correction" / "iter_16_verified"
    ...
    iter16_changes = state["adjustments"][15]["changes"]
    ...
    for profile in profiles:
        ...
        for change in iter16_changes:
            ...
            before_lab = rgb2lab((before_rgb.astype(float) / 255.0).reshape(1, 1, 3))[0, 0]
            after_lab = rgb2lab((suggested.astype(float) / 255.0).reshape(1, 1, 3))[0, 0]
    ...
    fig.suptitle(
        "Iteration 16 verification: iter 15 verified vs. iter 16 verified (worse entries reverted)",
        fontsize=12,
    )
