#!/usr/bin/env python3
"""Test high-saturation untested figures from existing papers."""
...
AUDIT_DIR = Path("runs/new_papers_vlm/quality_review_audit_untested_vivid")
...
def process(img_path: Path, desc: str, n_hint: int, engine: str | None = None, smooth: bool = False) -> dict | None:
    ...
    out_name = f"{desc.replace(' ', '_').replace('/', '_')}_n{n_hint}_{engine_str}_{n_found}l{smooth_str}_untested.png"
...
def main() -> None:
    ...
    # High-saturation untested candidates
    candidates = [
        # ml_velocity page12 - very vivid (sat=0.63), untested
        ("papers_new/to_process/ml_velocity_2024/ml_velocity_2024_page12_img0.png", "ml_velocity p12i0", [5, 6]),
        ("papers_new/to_process/ml_velocity_2024/ml_velocity_2024_page12_img1.png", "ml_velocity p12i1", [5, 6]),
        # ml_velocity page10 img4-7 - moderate vividness, not fully tested
        ("papers_new/to_process/ml_velocity_2024/ml_velocity_2024_page10_img4.png", "ml_velocity p10i4", [5, 6]),
        ("papers_new/to_process/ml_velocity_2024/ml_velocity_2024_page10_img5.png", "ml_velocity p10i5", [5, 6]),
        # wise_fwi page5 img1-2, page7 img1 - very vivid, large
        ("papers_new/to_process/wise_fwi_2024/wise_fwi_2024_page5_img1.png", "wise_fwi p5i1", [5, 6]),
        ("papers_new/to_process/wise_fwi_2024/wise_fwi_2024_page7_img1.png", "wise_fwi p7i1", [5, 6]),
        # tomography page5 img1-2, page7 img2-3 - moderate vividness
        ("papers_new/to_process/tomography_review_2024/tomography_review_2024_page5_img1.png", "tomography p5i1", [4, 5]),
        ("papers_new/to_process/tomography_review_2024/tomography_review_2024_page7_img2.png", "tomography p7i2", [4, 5]),
    ]
    ...
    for fname, desc, n_hints in candidates:
        ...
        if not img_path.exists():
            ...
            continue
        for n in n_hints:
            for eng in ["kmeans_full", "edge_grow"]:
                print(f"\nTesting {desc} n={n} eng={eng}...")
    ...
    report_path = AUDIT_DIR / "untested_vivid_report.json"
