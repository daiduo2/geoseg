#!/usr/bin/env python3
"""Verify iteration 14 palette adjustments and revert entries that worsened color fidelity."""
...
def main():
    ...
    before_dir = ref_root / "workflow_color_correction" / "iter_13_verified"
    after_dir = ref_root / "workflow_color_correction" / "iter_14"
    out_dir = ref_root / "workflow_color_correction" / "iter_14_verified"
    ...
    iter14_changes = state["adjustments"][13]["changes"]

    # Visual review decisions for iteration 14 adjustments.
    # Entries whose new color moves the region away from the original are reverted.
    # Diagnostic Delta-E and RGB L2 residuals were reviewed; the two top dark-red
    # entries clearly diverged from the brighter original and are reverted.
    revert_set = {
        ("fig6_profile_03", 0),
        ("fig6_profile_07", 0),
    }
    ...
    for profile in profiles:
        ...
        current = before_palette.copy()
        original_lab = rgb2lab(original)

        for change in iter14_changes:
            ...
            keep_visual = (profile, lab) not in revert_set
            ...
            current[lab] = suggested if keep_visual else before_rgb

        final_palettes[profile] = current.tolist()
        before_l2[profile] = mean_rgb_l2(original, before_mask, valid)
    ...
    fig.suptitle(
        "Iteration 14 verification: iter 13 verified vs. iter 14 verified (worse entries reverted)",
        fontsize=12,
    )
