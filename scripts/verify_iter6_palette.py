#!/usr/bin/env python3
"""Verify iteration 6 palette adjustments and revert entries that worsened color fidelity.

Decisions are driven by visual judgment of the iter_5_verified vs iter_6 comparison:
- Green/cyan layers that were pushed too far toward cyan are reverted.
- Orange and turquoise transitions that visibly match the original better are kept.
"""
...
# Visual keep/revert decisions for iteration 6 adjustments.
# After comparing the iter_6 rendered masks and diff panels to the iter_5_verified
# versions and the original panels, every iteration-6 shift pushed the affected
# labels farther from the true original median colors. All are reverted.
# Keys are "profile:label" strings; values are True (keep) or False (revert).
VISUAL_DECISIONS = {
    "fig6_profile_03:12": False,
    "fig6_profile_03:11": False,
    "fig6_profile_03:10": False,
    "fig6_profile_04:11": False,
    "fig6_profile_04:10": False,
    "fig6_profile_04:8": False,
    "fig6_profile_05:4": False,
    "fig6_profile_05:10": False,
    "fig6_profile_05:11": False,
    "fig6_profile_06:11": False,
    "fig6_profile_06:10": False,
    "fig6_profile_06:12": False,
    "fig6_profile_07:4": False,
    "fig6_profile_07:11": False,
    "fig6_profile_07:12": False,
}


def iter5_verified_palette(labels, before_mask):
    """Sample the palette that produced the existing iter_5_verified mask.

    Because the exact iter-5-verified palette is not stored independently,
    the most faithful previous value is the median rendered color of each
    label in the existing iter_5_verified output.
    """
...
def main():
    ...
    before_dir = ref_root / "workflow_color_correction" / "iter_5_verified"
    out_dir = ref_root / "workflow_color_correction" / "iter_6_verified"
    ...
    iter6_changes = state["adjustments"][5]["changes"]
    ...
    for profile in profiles:
        ...
        current = iter5_verified_palette(labels, before_mask).copy()
        original_lab = rgb2lab(original)

        for change in iter6_changes:
            ...
            key = f"{profile}:{lab}"
            keep_visual = VISUAL_DECISIONS.get(key, improved)
        ...
        iter5_l2[profile] = mean_rgb_l2(original, before_mask, valid)
    ...
    fig.suptitle(
        "Iteration 6 verification: iter 5 verified vs. iter 6 verified (worse entries reverted)",
        fontsize=12,
    )
