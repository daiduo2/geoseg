"""Run split_label_by_color_components for panel_2 label 1 and save artifacts."""
...
def add_legend(img_rgb: np.ndarray, labels: np.ndarray) -> np.ndarray:
    ...
    for label_id in range(1, n + 1):
        ...
        draw.text((x + box_w + 6, y), f"Label {label_id}", fill=(255, 255, 255), font=font, stroke_width=2, stroke_fill=(0, 0, 0))
        y += box_h + spacing
...
def build_side_by_side(
    ...
    try:
        ...
        font = ImageFont.load_default()
    titles = ["Original", "Baseline mask (label 1)", "Split mask"]
...
def main() -> None:
    ...
    # Save outputs
    np.save(OUT_DIR / "split_k4_a300_labels.npy", split_labels)
    overlay = overlay_labels(img_rgb, split_labels)
    overlay_legend = add_legend(overlay, split_labels)
    Image.fromarray(overlay_legend).save(OUT_DIR / "split_k4_a300_overlay.jpg", quality=95)

    compare = build_side_by_side(img_rgb, labels, split_labels, TARGET_LABEL)
    Image.fromarray(compare).save(OUT_DIR / "split_k4_a300_compare.jpg", quality=95)
    ...
    metrics = {
        ...
        "output_overlay": str(OUT_DIR / "split_k4_a300_overlay.jpg"),
        "output_compare": str(OUT_DIR / "split_k4_a300_compare.jpg"),
    }
    (OUT_DIR / "split_k4_a300_metrics.json").write_text(json.dumps(metrics, indent=2) + "\n")
