#!/usr/bin/env python3
"""Verify iteration 7 palette adjustments and revert entries that worsened color fidelity."""
...
def iter6_verified_palette(labels, before_mask):
    """Sample the palette that produced the existing iter_6_verified mask."""
...
# Visual keep/revert decisions for iteration 7 adjustments.
# After reviewing per-label crops and overall comparison grid:
# - Keep cyan/teal shifts that clearly match original bottom layers.
# - Keep green shifts that reduce lime/yellow cast.
# - Revert subtle mint changes and profile_05/profile_07 changes that pushed too far.
VISUAL_DECISIONS = {
    "fig6_profile_03:12": True,
    "fig6_profile_03:8": False,
    "fig6_profile_03:10": True,
    "fig6_profile_04:12": True,
    "fig6_profile_04:8": False,
    "fig6_profile_04:10": True,
    "fig6_profile_05:12": False,
    "fig6_profile_05:10": False,
    "fig6_profile_05:4": False,
    "fig6_profile_06:12": True,
    "fig6_profile_06:11": False,
    "fig6_profile_06:10": False,
    "fig6_profile_07:12": False,
    "fig6_profile_07:13": True,
    "fig6_profile_07:10": False,
}
...
def main():
    ...
    before_dir = ref_root / "workflow_color_correction" / "iter_6_verified"
    out_dir = ref_root / "workflow_color_correction" / "iter_7_verified"
    ...
    iter7_changes = state["adjustments"][6]["changes"]
    ...
    for profile in profiles:
        ...
        for change in iter7_changes:
            ...
            # Generate per-label detail crop using actual before/after renders
            ...
            if bbox is not None:
                ...
                titles = ["Original", "Iter 6 verified", "Iter 7 verified"]
                images = [
        ...
        # Use the actual saved masks for L2 to avoid render/sampling drift.
        iter6_l2[profile] = mean_rgb_l2(original, before_mask, valid)
        ...
        # Reload saved mask for consistent L2
    ...
    fig.suptitle(
        "Iteration 7 verification: iter 6 verified vs. iter 7 verified (worse entries reverted)",
        fontsize=12,
    )
