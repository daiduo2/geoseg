#!/usr/bin/env python3
"""Verify iteration 13 palette adjustments and revert entries that worsened color fidelity."""
...
def main():
    ...
    before_dir = ref_root / "workflow_color_correction" / "iter_12_verified"
    after_dir = ref_root / "workflow_color_correction" / "iter_13"
    out_dir = ref_root / "workflow_color_correction" / "iter_13_verified"
    ...
    iter13_changes = state["adjustments"][12]["changes"]

    # Visual review decisions for iteration 13 adjustments.
    # Entries that clearly moved the region away from the original are reverted;
    # the rest are kept.
    revert_set = {
        ("fig6_profile_03", 12),
        ("fig6_profile_03", 8),
    ...
    fig.suptitle(
        "Iteration 13 verification: iter 12 verified vs. iter 13 verified (worse entries reverted)",
        fontsize=12,
    )
