#!/usr/bin/env python3
"""Push borderline candidates over 0.7 with targeted parameter tuning."""
...
AUDIT_DIR = Path("runs/new_papers_vlm/quality_review_audit_pushover")
...
def process(img_path: Path, desc: str, n_hint: int, engine: str | None = None, panel_id: int | None = None) -> dict | None:
    ...
    out_name = f"{desc.replace(' ', '_').replace('/', '_')}_n{n_hint}_{engine_str}_{n_found}l_pushover.png"
...
def main() -> None:
    ...
    # ml_velocity p10i1 - 0.75 was close. Try fewer layers + smoother engine
    for n in [3, 4]:
        for eng in ["edge_grow", "kmeans_full"]:
            r = process(Path("papers_new/to_process/ml_velocity_2024/ml_velocity_2024_page10_img1.png"), "ml_velocity p10i1", n, eng)
            if r:
                all_results.append(r)
                print(f"ml_velocity p10i1 n={n} eng={eng}: score={r.get('score', 'ERR')} rec={r.get('recommendation', 'ERR')}")

    # tomography_review p5i1 panel 0 - very high sat/max_diff, small panel
    for n in [3, 4]:
        for eng in ["edge_grow", "kmeans_full"]:
            r = process(Path("papers_new/to_process/tomography_review_2024/tomography_review_2024_page5_img1.png"), "tomography p5i1", n, eng, 0)
            if r:
                all_results.append(r)
                print(f"tomography p5i1 n={n} eng={eng}: score={r.get('score', 'ERR')} rec={r.get('recommendation', 'ERR')}")

    # tomography_review p8i2 panel 0 - highest max_diff from screening
    for n in [4, 5]:
        for eng in ["edge_grow", "kmeans_full"]:
            r = process(Path("papers_new/to_process/tomography_review_2024/tomography_review_2024_page8_img2.png"), "tomography p8i2", n, eng, 0)
            if r:
                all_results.append(r)
                print(f"tomography p8i2 n={n} eng={eng}: score={r.get('score', 'ERR')} rec={r.get('recommendation', 'ERR')}")

    # ml_velocity p26i2 - high sat, smaller image
    for n in [4, 5]:
        for eng in ["edge_grow", "kmeans_full"]:
            r = process(Path("papers_new/to_process/ml_velocity_2024/ml_velocity_2024_page26_img2.png"), "ml_velocity p26i2", n, eng)
            if r:
                all_results.append(r)
                print(f"ml_velocity p26i2 n={n} eng={eng}: score={r.get('score', 'ERR')} rec={r.get('recommendation', 'ERR')}")

    report_path = AUDIT_DIR / "pushover_report.json"
