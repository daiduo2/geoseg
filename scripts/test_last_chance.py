#!/usr/bin/env python3
"""Last chance: try to push borderline candidates over 0.7."""
...
AUDIT_DIR = Path("runs/new_papers_vlm/quality_review_audit_last")
...
def process(img_path: Path, desc: str, n_hint: int, engine: str | None = None, panel_id: int | None = None) -> dict | None:
    ...
    out_name = f"{desc.replace(' ', '_').replace('/', '_')}_n{n_hint}_{engine_str}_{n_found}l_last.png"
...
def main() -> None:
    ...
    # fwi_deeponet page1_img3: was 0.78 with 3 layers. Try more layers.
    for n in [5, 6]:
        for eng in [None, "edge_guided", "kmeans_full"]:
            r = process(Path("papers_new/to_process/fwi_deeponet_2024/fwi_deeponet_2024_page1_img3.png"), "fwi_deeponet p1i3", n, eng)
            if r:
                all_results.append(r)
                print(f"fwi_deeponet p1i3 n={n} eng={eng}: score={r.get('score', 'ERR')} rec={r.get('recommendation', 'ERR')}")

    # tomography_review page5_img2: was 0.62 with 8 layers edge_guided. Try fewer layers + different engines.
    for n in [4, 5]:
        for eng in [None, "edge_grow", "kmeans_full"]:
            r = process(Path("papers_new/to_process/tomography_review_2024/tomography_review_2024_page5_img2.png"), "tomography p5i2", n, eng, 2)
            if r:
                all_results.append(r)
                print(f"tomography p5i2 n={n} eng={eng}: score={r.get('score', 'ERR')} rec={r.get('recommendation', 'ERR')}")

    # uncertainty_fwi page12_img9: was 0.60 with 3 layers edge_guided. Try more layers.
    for n in [4, 5]:
        for eng in [None, "edge_guided"]:
            r = process(Path("papers_new/to_process/uncertainty_fwi_2024/uncertainty_fwi_2024_page12_img9.png"), "uncertainty p12i9", n, eng)
            if r:
                all_results.append(r)
                print(f"uncertainty p12i9 n={n} eng={eng}: score={r.get('score', 'ERR')} rec={r.get('recommendation', 'ERR')}")

    report_path = AUDIT_DIR / "last_report.json"
