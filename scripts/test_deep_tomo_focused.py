#!/usr/bin/env python3
"""Test deep_tomo_2024 focused on large-panel images (page7,9,11)."""
...
AUDIT_DIR = Path("runs/new_papers_vlm/quality_review_audit_deeptomo_focused")
...
def process(img_path: Path, desc: str, n_hint: int, engine: str | None = None, panel_id: int | None = None) -> dict | None:
    ...
    out_name = f"{desc.replace(' ', '_').replace('/', '_')}_n{n_hint}_{engine_str}_{n_found}l_deeptomo.png"
...
def main() -> None:
    ...
    base = Path("papers_new/to_process/deep_tomo_2024")

    # Focus on large-panel images only
    candidates = [
        ("deep_tomo_2024_page7_img0.png", "deep_tomo p7i0", [5, 6]),
        ("deep_tomo_2024_page7_img1.png", "deep_tomo p7i1", [5, 6]),
        ("deep_tomo_2024_page9_img0.png", "deep_tomo p9i0", [5, 6]),
        ("deep_tomo_2024_page11_img0.png", "deep_tomo p11i0", [5, 6]),
    ]
        ...
        for n in n_hints:
            for eng in [None, "edge_grow"]:
                ...
                if r:
                    ...
                    print(f"  -> SKIP (too small or no reps)")

    report_path = AUDIT_DIR / "deeptomo_focused_report.json"
