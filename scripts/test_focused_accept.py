#!/usr/bin/env python3
"""Focused test on highest-probability ACCEPT candidates."""
...
AUDIT_DIR = Path("runs/new_papers_vlm/quality_review_audit_focused")
...
def process(img_path: Path, desc: str, n_hint: int, engine: str | None = None, panel_id: int | None = None) -> dict | None:
    ...
    out_name = f"{desc.replace(' ', '_').replace('/', '_')}_n{n_hint}_{engine_str}_{n_found}l_focused.png"
...
def main() -> None:
    ...
    # Top candidates - only test most promising combos
    test_cases = [
        # ml_velocity - high sat + high max_diff, reasonable size
        ...
        # tomography_review - very high sat, small panels but sharp boundaries
        ("papers_new/to_process/tomography_review_2024/tomography_review_2024_page5_img1.png", "tomography p5i1", [3, 4, 5], 0),
        ("papers_new/to_process/tomography_review_2024/tomography_review_2024_page8_img2.png", "tomography p8i2", [4, 5, 6], 0),
        # wise_fwi - high sat but huge images, try scaled
    ...
    report_path = AUDIT_DIR / "focused_report.json"
