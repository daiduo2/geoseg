#!/usr/bin/env python3
"""Test promising untested high-saturation images."""
...
AUDIT_DIR = Path("runs/new_papers_vlm/quality_review_audit_untested")
...
def process(img_path: Path, desc: str, n_hint: int, engine: str | None = None, panel_id: int | None = None) -> dict | None:
    ...
    out_name = f"{desc.replace(' ', '_').replace('/', '_')}_n{n_hint}_{engine_str}_{n_found}l_untested.png"
...
def main() -> None:
    ...
    # Scale down wise_fwi if needed
    def maybe_scale(path_str: str, max_dim: int = 2000) -> Path:
        p = Path(path_str)
        img = Image.open(p)
        w, h = img.size
        if w > max_dim:
            ...
            scaled = Path(f"/tmp/{p.stem}_scaled.png")
            img.save(scaled)
            return scaled
        return p

    test_cases = [
        ("papers_new/to_process/ml_velocity_2024/ml_velocity_2024_page21_img2.png", "ml_velocity p21i2", [5, 6]),
        ("papers_new/to_process/ml_velocity_2024/ml_velocity_2024_page26_img3.png", "ml_velocity p26i3", [5, 6]),
        ("papers_new/to_process/tomography_review_2024/tomography_review_2024_page9_img3.png", "tomography p9i3", [5, 6], 2),
        ("papers_new/to_process/wise_fwi_2024/wise_fwi_2024_page7_img0.png", "wise_fwi p7i0", [5, 6]),
    ]

    for img_path_str, desc, n_hints, *rest in test_cases:
        img_path = Path(img_path_str)
        panel_id = rest[0] if rest else None

        # Scale if wise_fwi
        if "wise_fwi" in desc:
            img_path = maybe_scale(img_path_str)
        else:
            img_path = Path(img_path_str)
        ...
        for n in n_hints:
            for eng in [None, "kmeans_full"]:
                r = process(img_path, desc, n, eng, panel_id)
                if r:
                    all_results.append(r)
                    print(f"{desc} n={n} eng={eng}: score={r.get('score', 'ERR')} rec={r.get('recommendation', 'ERR')}")

    report_path = AUDIT_DIR / "untested_report.json"
