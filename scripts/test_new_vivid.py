#!/usr/bin/env python3
"""Test newly found high-saturation candidates."""
...
AUDIT_DIR = Path("runs/new_papers_vlm/quality_review_audit_newvivid")
...
def process_and_review(img_path: Path, desc: str, n_hint: int = 5) -> list[dict]:
    print(f"\n{'='*60}")
    ...
    out_name = f"{desc.replace(' ', '_').replace('/', '_')}_panel{pb['id']}_{n_found}layers_newvivid.png"
    ...
    try:
        ...
        print(f"  -> ERROR: {exc}")
        return [{"desc": desc, "error": str(exc), "file": out_name}]
...
def main() -> None:
    ...
    test_cases = [
        ("papers_new/to_process/uncertainty_fwi_2024/uncertainty_fwi_2024_page12_img9.png", "uncertainty p12i9", 3),
        ("papers_new/to_process/uncertainty_fwi_2024/uncertainty_fwi_2024_page12_img7.png", "uncertainty p12i7", 3),
        ("papers_new/to_process/ml_velocity_2024/ml_velocity_2024_page24_img2.png", "ml_velocity p24i2", 6),
        ("papers_new/to_process/ml_velocity_2024/ml_velocity_2024_page26_img2.png", "ml_velocity p26i2", 8),
    ]
    ...
    report_path = AUDIT_DIR / "newvivid_report.json"
