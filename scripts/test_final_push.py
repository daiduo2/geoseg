#!/usr/bin/env python3
"""Final push: try everything to get ml_velocity and wise_fwi over 0.7."""
...
AUDIT_DIR = Path("runs/new_papers_vlm/quality_review_audit_finalpush")
...
def process(img_path: Path, desc: str, n_hint: int, engine: str | None = None, panel_id: int | None = None, preprocess: str | None = None) -> dict | None:
    img_rgb = np.array(Image.open(img_path).convert("RGB"))

    # Optional preprocessing
    if preprocess == "gaussian":
        from scipy import ndimage
        img_rgb = ndimage.gaussian_filter(img_rgb.astype(float), sigma=1.0).astype(np.uint8)
    elif preprocess == "median":
        from scipy import ndimage
        img_rgb = ndimage.median_filter(img_rgb, size=3)
    ...
    prep_str = f"_{preprocess}" if preprocess else ""
    out_name = f"{desc.replace(' ', '_').replace('/', '_')}_n{n_hint}_{engine_str}_{n_found}l{prep_str}_finalpush.png"
    ...
    try:
        ...
        return {"desc": desc, "n_hint": n_hint, "engine": engine_str, "n_found": n_found, "preprocess": preprocess, "error": str(exc)}
...
def main() -> None:
    ...
    # ml_velocity p10i1 - our best candidate at 0.75
    # Try all engines including ensemble and edge_grow
    for eng in ["ensemble", "edge_grow", "kmeans_full"]:
        for prep in [None, "gaussian"]:
            r = process(Path("papers_new/to_process/ml_velocity_2024/ml_velocity_2024_page10_img1.png"), "ml_velocity p10i1", 6, eng, preprocess=prep)
            if r:
                all_results.append(r)
                print(f"ml_velocity p10i1 n=6 eng={eng} prep={prep}: score={r.get('score', 'ERR')} rec={r.get('recommendation', 'ERR')}")

    # wise_fwi - scale down huge images first
    img = Image.open("papers_new/to_process/wise_fwi_2024/wise_fwi_2024_page5_img0.png")
    w, h = img.size
    if w > 2000:
        scale = 2000 / w
        new_w, new_h = int(w * scale), int(h * scale)
        img = img.resize((new_w, new_h), Image.LANCZOS)
        scaled_path = Path("/tmp/wise_fwi_p5i0_scaled.png")
        img.save(scaled_path)
        for eng in [None, "kmeans_full", "edge_grow"]:
            r = process(scaled_path, "wise_fwi p5i0_scaled", 5, eng)
            if r:
                all_results.append(r)
                print(f"wise_fwi p5i0_scaled n=5 eng={eng}: score={r.get('score', 'ERR')} rec={r.get('recommendation', 'ERR')}")

    report_path = AUDIT_DIR / "finalpush_report.json"
