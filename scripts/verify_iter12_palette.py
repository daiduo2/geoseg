#!/usr/bin/env python3
"""Verify iteration 12 palette adjustments and revert entries that worsened color fidelity."""
...
def main():
    ...
    before_dir = ref_root / "workflow_color_correction" / "iter_11"
    after_dir = ref_root / "workflow_color_correction" / "iter_12"
    out_dir = ref_root / "workflow_color_correction" / "iter_12_verified"
    ...
    iter12_changes = state["adjustments"][11]["changes"]

    # Visual review of the iter 12 comparison grid showed that every adjustment
    # moved its region away from the original color. Revert all iter 12 changes.
    revert_set = {(ch["profile"], int(ch["label"])) for ch in iter12_changes}
    ...
    fig.suptitle(
        "Iteration 12 verification: iter 11 vs. iter 12 verified (worse entries reverted)",
        fontsize=12,
    )
